...
function render() {
    var actualCurrentTime = new Date(); // Get the real current time

    // Calculate how many full days have passed since the page was loaded
    var currentDateNormalized = new Date(actualCurrentTime.getFullYear(), actualCurrentTime.getMonth(), actualCurrentTime.getDate());
    currentDateNormalized.setHours(0, 0, 0, 0); // Normalize current date to its beginning
    var millisecondsPerDay = 24 * 60 * 60 * 1000;
    var daysPassed = (currentDateNormalized.getTime() - scriptLoadDateNormalized.getTime()) / millisecondsPerDay;

    var dailyAccumulatedOffsetms = 0;
    // Use config.ofs (parsed float, defaults to 0)
    if (!isNaN(config.ofs) && config.ofs !== 0) {
        dailyAccumulatedOffsetms = daysPassed * config.ofs;
    }
    
    // Apply the accumulated offset to the current time
    var time = new Date(actualCurrentTime.getTime() + dailyAccumulatedOffsetms * 1); // This 'time' is the adjusted local time base
    var len = time.getTime();
    var offset = time.getTimezoneOffset() * 60000;
    var utcTime = len + offset;
    var date = new Date(utcTime + 3600000 * config.timezone);
...
var config = {
    ...
    rotate: urlQuery.r,
    lang: urlQuery.l
    ofs: parseFloat(urlQuery.ofs || 0) // Add new URL param, default to 0
};
